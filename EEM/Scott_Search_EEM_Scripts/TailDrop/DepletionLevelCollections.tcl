::cisco::eem::event_register_syslog pattern "Depletion level .* for resource PARTICLE HEADER" maxrun 600

###################################################################
# Trigger syslog events:
# ----------------------------------------------------------
# Depletion level 89 percent for resource PARTICLE HEADER
###################################################################

#------------------------------------------------------------------
# If a Depletion Level generated this script is run
#
...
#set run_flag "harddisk:/eem/DepletionLevel_run_flag"
...
set filename "DepletionLevel.$date_time"
...
#  set msg "EEM policy DepletionLevel.tcl was triggered, however, the run_flag file: $run_flag exists"
#  action_syslog msg $msg
#
#  set msg "$msg\nDelete $run_flag in order to activate the EEM DepletionLevel.tcl script."
#  set msg "$msg\n$syslog_msg"
#  puts $FH $msg
#  close $FH
#  exit
...
# Get JobID
set CMD "show process block location $location"
if [catch {cli_exec $cli(fd) $CMD} result] {
  error $result $errorInfo
}
puts $FH "$result\n\n"

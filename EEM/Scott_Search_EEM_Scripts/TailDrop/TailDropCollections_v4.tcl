...
#::cisco::eem::event_register_syslog pattern "Taildrop on .* queue .* by netio" maxrun 600
...
foreach ID $jobIDs {
  if {$ID == ""} { continue }
  set CMD "show packet-memory job $ID location $location"
  if [catch {cli_exec $cli(fd) $CMD} result] {
    error $result $errorInfo
  }
  puts $FH "$result\n\n"

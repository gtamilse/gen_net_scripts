...
#
# EEM script will be triggered off the above syslog message.  If a FortyGigE interface goes down this eem script
# will shutdown the mapped TE dummy interface
#
# Below environment varibales need to be set within the XR config:
# set _storage_location "disk0:/eem"
# set _down_40G_output_log "Down_40G_bw_update"
...
action_syslog msg "EEM policy Down_40G_bw_update has started"
...
if {![info exists _down_40G_output_log]} {
  set result "**ERROR: EEM policy error environment var _down_40G_output_log not set"
  puts $result
  action_syslog msg $result
  exit 1
}
...
proc determine_tunnel_dummy {Interface FH} {
  ...
  if {![file exists $mapping_file]} {
    action_syslog msg "**ERROR: EEM policy Down_40G_bw_update 40Gig mappings file unknown or does not exist"
    exit 1
  }
...
set _down_40G_output_log "${_down_40G_output_log}_$stamp"
...
if [catch {open $_storage_location/$_down_40G_output_log w} result] {
    error $result
}
set FH $result

puts $FH "*Timestamp = $date"
puts $FH "Cisco Down_40G_bw_update"
...
if {$dummy_interface != ""} {
  puts $FH "Shutdown Tunnel Dumy Interface: $dummy_interface\n"
  ...
  action_syslog msg "EEM policy Down_40G_bw_update shutdwon following Tunnel Dummy interface: $dummy_interface"
} else {
  action_syslog msg "**ERROR: EEM policy Down_40G_bw_update unable to determine the Tunnel Dummy Interface"
}

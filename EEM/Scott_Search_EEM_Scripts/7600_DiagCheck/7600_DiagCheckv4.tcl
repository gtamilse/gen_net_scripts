::cisco::eem::event_register_timer cron name test cron_entry "* * * * *" maxrun_sec 180
...
#
# Description:
#   The EEM script is run in a crontab mode.  The script runs ever 1 minute and checks the diagnostic consecutive
#   failure count value.  The script will collect show commands and then execute a RSP switchover if either of the below conditions is true
...
if {$triggerFO == 1} {
  ...
  if [catch {cli_write $cli(fd) "redundancy force-switchover"} result] {
    error $result $errorInfo
  }
  puts $FH "$result\n"

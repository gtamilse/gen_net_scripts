::cisco::eem::event_register_timer cron name test cron_entry "0,5,10,15,20,25,30,35,40,45,50,55 * * * *" maxrun_sec 180
...
#
# Description:
#   The EEM script is run in a crontab mode.  The script runs ever 5 minutes and checks the diagnostic consecutive
#   failure count value.  If the value is 10 or greater the script continues and verifies the failure count for 
#   modules 1 and 2.  If there values are non-zero the script collects a number of show commands before executing
#   a RSP switchover
#
# Email Option
...
if {$continue >= 10} {
  # Consecutive failure count for Active RP found to be >= 10.  Collect futher commands
  if [catch {cli_exec $cli(fd) "show diagnostic result module 1 test TestFabricFlowControlStatus detail"} result1] {
    error $result1 $errorInfo
  }
  if [catch {cli_exec $cli(fd) "show diagnostic result module 1 test TestFabricFlowControlStatus detail"} result2] {
  ...
  if {$mod1Count >= 1 && $mod2Count >= 1} {
    ...
    # EEM script found both modules 1 and 2 consecutive failure counts non-zero and will now collect additional
    # captures and eventually perform a switchover
    ...
    # Running the list of capture commands before RSP switchover
    ...
    set msg "EEM script 7600_DiagCheck ($node) Executed.  EEM script captures saved to file:\n$output_file"
    action_syslog msg $msg

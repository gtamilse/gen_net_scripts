::cisco::eem::event_register_syslog pattern "SSHD.*SECURITY-SSHD-6-INFO_SUCCESS.*Successfully authenticated user.*aracscbb" maxrun 600
...
# Example pattern:
# ----------------
#  RP/0/7/CPU0:Jul 24 05:36:19.316 : SSHD_[65744]: %SECURITY-SSHD-6-INFO_SUCCESS : Successfully authenticated user 'aracscbb' 
...
# Next need to enable debugging.  But first the script will need to turn on 'logging buffered debug' so the debugs
# are saved to syslogd
...
# Perform the 'commit confirm'
if [catch {cli_write $cli(fd) "commit confirm 180"} result] {
  error $result $errorInfo
}

# Now the router may have another user in config mode or ran commit.  In this case the script will need to 
# be able to catch this and answer yes if needed. Example:
# Do you wish to exit? [no]: yes
if [catch {cli_read_pattern $cli(fd) "\[no\]:|\[yes\]:|RP\/0\/.*config"} result] {
  error $result $errorInfo
}

if [catch {cli_write $cli(fd) "yes"} result] {
  error $result $errorInfo
}
...
foreach cmd [split $debug_cmds "\n"] {
  ...
  if {$cmd != ""} {
    puts $FH "Running CMD: $cmd"
    if [catch {cli_exec $cli(fd) "do $cmd"} result] {
      error $result $errorInfo
    }
  }
}
...
# sleep for 185 seconds.  Then exit from config mode.
sleep 185 
...
;# "show log | inc \"SSHD|xml\" | inc \"$month_day\""

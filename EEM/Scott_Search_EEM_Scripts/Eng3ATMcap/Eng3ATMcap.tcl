...
############################################################
set show_cmds "run show_psarb_trace -i all
...
puts $FH "OUTPUT for SHELL (ls /net):\n$result"


if [catch {cli_write $cli(fd) "ls /net/$nodename"} result] {
  error $result $errorInfo
}
if [catch {cli_read_pattern $cli(fd) "#"} result] {
  error $result $errorInfo
...
puts $FH "OUTPUT for SHELL (ls /net/$nodename):\n$result"
...
puts $FH "OUTPUT for (cat /proc/qnetstats*):\n$result"
...
puts $FH "OUTPUT for (pidin -p 1 fds):\n$result"
...
puts $FH "OUTPUT for (show_psarb_listener -c -l all -A):\n$result"

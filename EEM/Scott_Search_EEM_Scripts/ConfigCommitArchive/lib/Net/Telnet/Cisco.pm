...
# Mod History:
# ------------
# 09/02/09 -ssearch: Use of /g modifier is meaningless in split
#
# 10/30/09 -ssearch: Updated the login with more username prompt settings
#          -ssearch: Updated the cmd() to not die if the router returns an invalid
#                    command.
#
# 02/25/10 -ssearch: Updated the login with even more optional expected cli's
#
# 05/18/10 -ssearch: Updated the failed login output.
#
#-----------------------------------------------------------------
...
sub cmd {
	...
	for ( my ($i, $lastline) = (0, '');
	    ...
	    # This may have to be a pattern match instead.
            ...
            # ssearch: Commenting out the below since we do not want to kill/die
            #          the original script calling Cisco.pm when a Invalid command
            #          or input detected/protocol not configured.  Instead continue
            #          on with the original script.

	    if ( ( substr $out[$i], 0, 1 ) eq '%' ) {
		if ( $out[$i] =~ /'\^' marker/ ) { # Typo & bad arg errors
                  # Below is for unexpected command/not supported;
                  my $ss_error = "% Invalid input detected";
                  my $msg = "Unexpected ERROR from command: $cmd \n$ss_error\n";
                  @out = $msg;
                  #print $msg;
		#    chomp $lastline;
		#    $self->error( join "\n",
		...
		#    splice @out, $i - 1, 3;
                  ...
                  # Below is for $cmd not running/protocol not running
                  my $msg = "Unexpected ERROR from command: $cmd \n$out[$i]\n";
                  @out = $msg;
                  #print $msg
		#    chomp $out[$i];
...
sub login {
	...
	my $cmd_prompt_re = $self->re_sans_delims($cmd_prompt);
        ...
        # ssearch: For a successful login we need to see the following prompt:
        $cmd_prompt_re = '.*(#|# |>|> )$';
        #print "{DEBUGGING} cmd_prompt_re: $cmd_prompt_re\n";
        #print "\n{DEBUGGING} Prompt - $_\n";

	if (not defined) {
	    #return $self->error("ss5 - login failed: access denied or bad name, passwd, etc");
	    die "ss5 - login failed: Access denided or bad Username and/or Password\n";
	} elsif (/sername|ogin/) {
            #print "ssearch debugging: HERE 1\n";
	    $self->print($username) or return &$error("login disconnected");
	    #$seen{login}++ && $self->error("ss6 - login failed: access denied or bad username");
	    $seen{login}++ && die "ss6 - Login failed, bad Username and/or Password\n";
	} elsif (/[Pp]assw/) {
            #print "ssearch debugging: HERE 2\n";
	    $self->print($password) or return &$error("login disconnected");
	    #$seen{passwd}++ && $self->error("ss7 - login failed: access denied or bad password");
	    $seen{passwd}++ && die "ss7 - Login failed, bad Username and/or Password\n";
        #################################
        # ssearch: Begin
        } elsif (/NOC:$/) {
            #print "ssearch debugging: HERE 3\n";
            $self->print($username) or return &$error("login disconnected");
            #$seen{login}++ && $self->error("ss1 login failed: access denied or bad username");
            $seen{login}++ && die "ss1 Login failed, bad Username and/or Password\n";
        } elsif (/NOC: $/) {
            #print "ssearch debugging: HERE 4\n";
            $self->print($username) or return &$error("login disconnected");
            #$seen{login}++ && $self->error("ss2 login failed: access denied or bad username");
            $seen{login}++ && die "ss2 Login failed, bad Username and/or Password\n";
#        } elsif (/:$/) {
#            #print "ssearch debugging: HERE 5\n";
#            $self->print($username) or return &$error("login disconnected");
#            #$seen{login}++ && $self->error("ss3 login failed: access denied or bad username");
#            $seen{login}++ && die "ss3 Login failed, bad Username and/or Password\n";
#        } elsif (/: $/) {
#            #print "ssearch debugging: HERE 6\n";
#            $self->print($username) or return &$error("login disconnected");
#            #$seen{login}++ && $self->error("ss4 login failed: access denied or bad username");
#            $seen{login}++ && die "ss4 Login failed, bad Username and/or Password\n";
#        # ssearch: End
        #################################
	} elsif (/(?i:Passcode)/) {
	    $self->print($passcode) or return &$error("login disconnected");
	    #$seen{passcode}++ && $self->error("ss8 - login failed: access denied or bad passcode");
            $seen{passcode}++ && die "ss8 Login failed, bad Username and/or Passcode\n";
	} elsif (/($cmd_prompt_re)/) {
            #print "\nssearch debugging: At success. cmd_prompt_re: $cmd_prompt_re\n\n";
	    &$reset; # Success. Reset obj to default vals before continuing.
	    last;
	} else {
	    #$self->error("login received unexpected prompt. Aborting.");
	    die "Login received unexpected prompt. Aborting.\n";
	}
    }

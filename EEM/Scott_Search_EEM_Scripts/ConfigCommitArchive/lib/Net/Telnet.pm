...
# Updates:
# -----------
# 10/30/09 -ssearch: made updates to maxbufsize 
#                    And updated the sub max_buffer_length to take the maxbufsize
#                    as the default (search on: ssearch).
# 11/12/10 -ssearch: Made a change to the _fillbuff local routine below to remove any
#                    ending line spaces. This fixes some of the expected prompt returns
# 11/12/10 -ssearch: Created a new object->rdump. If the user specifies the rdump option
#                    the tool will now display the actual router real-time communication.
#
...
sub new {
    ...
    *$self->{net_telnet} = {
	...
	foreach (keys %args) {
		...
		$self->rdump($args{$_});
                #print "ssearch: $_\n";
                #print "ssearch: rdump($args{$_})\n";
                #print "ssearch: $self->rdump($args{$_})\n";
	    }
...
#	}
#
        # ssearch: Hardcoded the maxbufsize value below:
        $maxbufsize = 5000000000;
...
sub _fillbuf {
	...
	next if $unparsed_pos >= length $s->{buf};
        ...
        #ssearch88
        # The below line will print all communications with the router in real time:
        #print "HERE $s->{rdump}\n";
        if ($s->{rdump}) {
          # Below is to void out displaying the users password during a 'copy ftp:' command:
          if ($s->{buf} !~ /copy ftp:|secret /i) {
            print $ST{magenta}, substr($s->{buf}, $unparsed_pos), $ST{reset};
          }
        }

        # ssearch88: Below is to remove a trailing space if one if found:
        #if ($s->{buf} =~ / $/) {
        #  $s->{buf} =~ s/\s+$//;
        #}

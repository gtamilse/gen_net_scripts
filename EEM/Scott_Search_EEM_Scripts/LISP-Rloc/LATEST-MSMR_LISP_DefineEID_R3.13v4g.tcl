::cisco::eem::event_register_timer cron name LISP_DefineEID cron_entry "0-59/5 * * * *" maxrun_sec 600
#------------------------------------------------------------------
# MSMR_LISP_DefineEID_R3.13v3.tcl
#
# ABOUT ---------------------------------------------------
#  This EEM script runs 6 shows command, parses the data to
#   gather necessary information to determine if the script
#   should statically configure a new EID definition.
#  This EEM script run once every 6 hours.
#
#
# PRE-REQUISITES ------------------------------------------
#  1. EEM directory must exist in boothflash:eem
#  2. All event manager configurations must be defined.
#  3. EEM local user account must be defined.
#
#
# REVISION HISTORY ----------------------------------------
#
# R3.13 v1
#  Author: Scott Search (ssearch@cisco.com)
#  Date:   5/27/2014

#  Detail: Original version
#
# R3.13 v2
#  Author: Brian Oglesby (brian.oglesby@att.com)
#  Date:   6/26/2014
#  Detail: Updated the chron timer to run script every 6 hours.
#          Changed the file naming convention to add version scheme.
#          Updated the introduction comment structure
#          Commented out ^Qwrite memory^R to prevent other user configs from saving
#
# R3.13 v3
#  Editor: Brian Oglesby (brian.oglesby@att.com)
#  Date:   10/20/2014
#  Detail: Updated the maximum run timer from 3 to 5 seconds (maxrun_sec 600)
#          Added get_tag procedure to parse community into useable route-tag.
#          Removed get_community procedure which is replaced by get_tag.
#          Updated script procedure calls to use get_tag
#          Added a mechanism where log files will be maintained for a week then overwritten
#
# R3.13 v4
#  Editor: Toby Rees (trees@cisco.com)
#  Date:   11/11/2014
#  Detail: Updated the procedure verify_registered_prefixes
#          Enabled identification of IPv6 EID with non-0 last segment.
#
# R3.13 v4b
#  Editor: Toby Rees/Lingyu Pan (trees@cisco.com)
#  Date:   11/12/2014
#  Detail: debug & change the first line to run every 5 min
#	   update the matching regexp again
#          
#           
#
# R3.13 v4g
...
#  Date:   11/12/2014
#  Detail: debug & change the first line to run every 5 min
#	   update the matching regexp again to correctly match IPv6 EID.
#      Fixed the commenting out of the "Write Memory" that prevented all prefixed being processed.    
...
# Value is intended to have an _ in front
...
proc verify_registered_prefixes {output} {
  ...
  foreach line [split $output "\n"] {
    ...
    if {$line != ""} {
      ...
      if {[regexp {[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+\/[0-9]+$} $line] || [regexp {\:\:\/[0-9]+$} $line] || [regexp {\:\:[0-9]+\/[0-9]+$} $line] } {
...
proc get_tag {output} {
  ...
  foreach line [split $output "\n"] {
    ...
    if {$line != ""} {
      ...
      if {$community != "NA"} {
          set tag [lindex [split $community ":"] end]
      }
      if {$tag != "NA"} {
        return $tag
      }
...
#Get run number for the day (1-4)
          ...
          foreach key [array names array1] {
            ...
            if { [ llength $InstanceID ] >= 1 && [ llength $EIDprefix ] >= 1} {
              ...
              if { [ llength $LocatorAddress ] >= 1 && $LocatorAddress != 0} {
                ...
                set output $result
                set tag [get_tag $output]
                puts $FH "Process $process Locator Address: $LocatorAddress tag: $tag"

                if { [ llength $tag ] >= 1 && $tag != "NA"} {
                  ...
                  puts $FH "site 0000"
                  set cmd "eid-prefix instance-id $InstanceID $EIDprefix route-tag $tag"
                  puts $FH "$cmd\n"

::cisco::eem::event_register_syslog pattern "hfa_main.*L2-PSE-6-INFO_MSG.*Invoking Soft Error Recovery" maxrun 3600

#------------------------------------------------------------------
# e3ATM_PlimError(2) EEM Script
#
# February 2011 - Scott Search (ssearch@cisco.com)
#
# EEM script/policy to detect the syslog pattern above in the event a xr12k node experiences 
# an engine 3 ATM LC PLIM ERROR.  This script will capture a number of commands for later analysis.
#
# EEM Script is dependent on the following event manager environment variable(s):
#   _e3ATM_PlimError_storage_location <storage>  -Disk/hardisk storage location "harddisk:/eem"
#
# Email Option
...
# The script captures numerous show command outputs from EXEC, ADMIN and the SHELL. The script captures
# the Exec and LC attach outputs 3 times with a 8 second sleep interval between.
#
...
set filename "e3ATM_PlimError2.$date_time"
...
set msg "EEM script e3ATM_PlimError ($node) detected a possible Engine 3 ATM LC failure and captured a number of commands"
